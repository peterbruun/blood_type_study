...
if (single_sex) {
   fit <- glm(age_at_diagnosis ~ rcs(Birth_year,knots = BIRTH_YEARS_KNOTS) + factor(exposure) + factor(AB0),
	data,family = "gaussian",maxit=200)
} else {
   fit <- glm(age_at_diagnosis ~ rcs(Birth_year,knots = BIRTH_YEARS_KNOTS) + factor(C_KON) + factor(exposure) + factor(AB0),
	data,family = "gaussian",maxit=200)
}
