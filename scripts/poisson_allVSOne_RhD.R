...
data <- data %>% mutate(exposure = if_else(Rhesus == "Positiv", 1, 0))
...
if (perinatal){
	...
	data <- data %>% group_by(Birth_year,exposure,AB0) %>% summarise(population = sum(pop), cases = sum(CASE))
	# Relative risk ratio
	fit_interact <- glm(cases ~ rcs(Birth_year,knots = BIRTH_YEARS_KNOTS) + factor(exposure) + factor(AB0) + offset(log(population)),
		data,family = quasipoisson(link = "log"),maxit=200)

	} else {
	# Group data into count table
	data <- data %>% group_by(Birth_year,C_KON,exposure,AB0) %>% summarise(population = sum(pop), cases = sum(CASE))
	# Relative risk ratio
	fit_interact <- glm(cases ~ rcs(Birth_year,knots = BIRTH_YEARS_KNOTS) + factor(C_KON) + factor(exposure) + factor(AB0) + offset(log(population)),
		data,family = quasipoisson(link = "log"),maxit=200)
	}
} else {
	if (single_sex) {
		# Only one sex
		# Group data into count table 
		data <- data %>% group_by(Birth_year,exposure,AB0,age_group) %>% summarise(pyrs = sum(pyrs), cases = sum(CASE))
		# Per 1000 person years
		data$pyrs <- data$pyrs/1000
		# Fit rcs(Birth_year,knots = BIRTH_YEARS_KNOTS)
		fit_interact <- glm(cases ~ rcs(Birth_year,knots = BIRTH_YEARS_KNOTS) + rcs(age_group,knots=AGE_KNOTS) + factor(exposure) + factor(AB0) + offset(log(pyrs)),
			data,family = quasipoisson(link = "log"),maxit=200)
	} else {
		# Both sex
		# Group data into count table 
		data <- data %>% group_by(Birth_year,exposure,AB0,C_KON,age_group) %>% summarise(pyrs = sum(pyrs), cases = sum(CASE))
		# Per 1000 person years
		data$pyrs <- data$pyrs/1000
		# Fit
		fit_interact <- glm(cases ~ rcs(Birth_year,knots = BIRTH_YEARS_KNOTS) + rcs(age_group,knots=AGE_KNOTS) + factor(C_KON) + rcs(age_group,knots=AGE_KNOTS):factor(C_KON) + factor(exposure) + factor(AB0) + offset(log(pyrs)),
			data,family = quasipoisson(link = "log"),maxit=200)
	}
}
